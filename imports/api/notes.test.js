...
if (Meteor.isServer) {
  describe('notes', function(){
    ...
    it('should throw error if extra updates', function(){
      ...
      expect(() => {
        Meteor.server.method_handlers['notes.update'].apply({userId: noteOne.userId}, [noteOne._id, {name}]);
      }).toThrow();
    });

    it('should not update note if user was not creator', function(){
      Meteor.server.method_handlers['notes.update'].apply({userId: 'wrongId'}, [
        noteOne._id, {title}
      ]);
    ...
    it('should not update note if unauthenticated', function(){
      expect(() => {
        Meteor.server.method_handlers['notes.update'].apply({}, [noteOne.userId, {title}]);
      }).toThrow();
    });
    ...
    it('should return zero notes for user with no notes', function(){
      const res = Meteor.server.publish_handlers.notes.apply({userId: 'bad id'});
      const notes = res.fetch();
